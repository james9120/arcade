...
SCENARIOS = tuple(os.environ.get('MOTION_SCENARIOS', 'chain,power,turn,wall,tap,low,imperfect').split(','))
...
if(kind==='wall'){g.setSwingMode('MANUAL');const b=g.buildings.find(b=>!b.base&&b.x>20&&b.x<100&&b.z>25&&b.z<70);g.setPosition([b.x-b.w/2-.7,25,b.z-b.d/2+2],[.8,0,26]);}
...
g.input.y=kind==='power'&&t<1.25?-1:kind==='low'&&t<1.5?-.7:0;
g.input.x=kind==='turn'&&t<1.9||kind==='imperfect'&&t<2.7?1:kind==='wall'&&t>.3&&t<.7?1:0;
if(kind==='turn'&&i===23){g.input.web=true;g.attach();}
if(kind==='power'&&i===14){g.input.web=false;g.release(false,true);}if(kind==='power'&&i===27)g.input.web=true;
if(kind==='wall'&&t>.7)g.input.web=true;
if(kind==='tap'&&i===5){g.fireTap(g.refreshTarget(),150,250);}if(kind==='imperfect'&&i===40)g.fireTap(null,100,100);
g.step(10);g.render(1/12,1);
return{t:g.stats.elapsed,state:g.state,p:g.P.p,v:g.P.v,rope:g.P.rope&&{age:g.P.rope.age,charge:g.P.rope.charge,length:g.P.rope.length},swings:g.stats.swingCount,wall:!!g.P.wall,wallRuns:g.wallControl.runs,glError:g.stats.glError};}"""
...
with sync_playwright() as p:
    ...
    for kind in SCENARIOS:
        ...
        states = []
        for i in range(48 if kind == 'turn' else 96):
