...
SCENARIOS = tuple(os.environ.get('MOTION_SCENARIOS', 'normal,chain,power,turn,wall,tap,low,imperfect').split(','))
...
if(kind==='normal'){g.setSwingMode('NORMAL');g.webPress();g.jumpDown();}
...
g.input.y=kind==='normal'&&t<1||kind==='power'&&t<1.25?-1:kind==='low'&&t<1.5?-.7:0;
