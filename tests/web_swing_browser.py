...
with sync_playwright() as playwright:
    for name in ('chromium', 'webkit'):
        ...
        try:
            ...
            page.locator('#web').tap()
            check(name, 'actual normal WEB tap latches after the finger lifts', page.evaluate("__webswing.settings.swingMode==='NORMAL' && !!__webswing.P.rope && !__webswing.input.web"))
            page.locator('#jump').tap()
            check(name, 'actual JUMP tap releases the normal web', page.evaluate('!__webswing.P.rope && __webswing.jumpControl.airUsed && !__webswing.jumpControl.held'))
            page.keyboard.press('Space')
            check(name, 'keyboard Space tap catches without immediate key-up release', page.evaluate('!!__webswing.P.rope && !__webswing.input.web'))
            page.keyboard.down('KeyX')
            page.evaluate('__webswing.step(108)')
            charged = page.evaluate('__webswing.jumpControl.charge')
            page.keyboard.up('KeyX')
            check(name, 'keyboard X charges and launches on release', charged == 1 and page.evaluate('!__webswing.P.rope && !__webswing.jumpControl.held'))
