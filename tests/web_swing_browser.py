...
SUITE = r'''() => {
 ...
 check('scripted catch-release-reel traversal crosses several blocks',()=>{fresh();let air=0,releases=0;for(let i=0;i<120*30&&g.state==='playing';i++){const {P,input}=g;input.x=Math.max(-.9,Math.min(.9,P.p[0]/15+P.v[0]/13));input.y=P.p[1]<45?-.5:0;if(P.rope){air=0;if(P.p[1]<20&&g.stats.energy>31)g.zip();if(P.rope.age>.35&&P.v[1]>3&&P.v[2]>8){g.release();releases++}}else{air+=1/120;if(i===0||air>.22&&(P.v[1]<1||P.p[1]<22))g.attach()}g.step()}const s=g.stats;assert(g.state==='playing','scripted run crashed');assert(s.elapsed>29.99,'did not finish 30 seconds');assert(s.swingCount>=16&&releases>=16,'not enough chained swings');assert(g.P.p[2]>748,'no sustained forward traversal');assert(s.overflows===0,'geometry overflow during traversal');return {seconds:s.elapsed,distance:s.distance,forwardProgress:g.P.p[2]-48,swings:s.swingCount,releases}});
...
with sync_playwright() as p:
    for name in ('chromium', 'webkit'):
        ...
        try:
            ...
            # Wait for WebKit's asynchronous viewport/layout and resize notification before
            # manually rendering. A paused test RAF must not screenshot stale portrait pixels.
